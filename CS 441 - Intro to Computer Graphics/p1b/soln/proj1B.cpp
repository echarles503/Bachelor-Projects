...
public:
	...
	double scanline(double m, int y)
	{
		if (isnan(m) || isinf(m))
			return X[0];
...
public:
	...
	void rasterizeTriUp(Triangle &t)
	{
		...
		{	
			int minCol = (int)C441(t.scanline(lSlope, i)); 
			int maxCol = (int)F441(t.scanline(rSlope, i));	
			
			//printf("%d : %d -> %d\n", i, minCol, maxCol);
			for (int j = minCol; j <= maxCol; j++)
			{	
				// Invert y axis (height-i-1)
				if ((h-1-i >= 0) && (h-1-i <= h-1) && (j <= w-1) && (j >= 0))
					setPixel(j, h-1-i, t.getColor());
			}
		}
