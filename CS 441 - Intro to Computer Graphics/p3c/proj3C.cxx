...
int main() 
{
    ...
    for (int i = 0; i < numBalls; i++)
    {
        ...
        std::uniform_real_distribution<> z_dis(-10, 10);
        std::uniform_real_distribution<> height_dis(10, 40);  
        double rand_height = height_dis(gen);
        b.x = x_dis(gen);
        b.z = z_dis(gen);
        b.BP.h0 = rand_height;
        b.BP.h = rand_height;
        b.BP.hmax = rand_height;
...
BounceBall(std::vector<Ball> &balls, RenderManager &rm, glm::vec3 camPos)
{
    ...
    int c1 = 0, c2 = 0, c3 = 0;
    for(auto& b : balls){
        b.distToCamera = sqrt(pow((camPos.x - b.x), 2) + pow((camPos.y - b.BP.h), 2) + pow((camPos.z - b.z), 2));
        //printf("cam: %f %f %f\n", camPos.x, camPos.y, camPos.z);
        //printf("ball: %f %f %f\n", b.x, b.BP.h, b.z);
        //printf("dist: %f\n", b.distToCamera);
        glm::mat4 identity(1.0f);
        glm::mat4 pos = TranslateMatrix(b.x, b.BP.h, b.z);
        rm.SetColor(.5, .6, .7);
        
        if (b.distToCamera <= 20)
        {
            // L3
            rm.Render(RenderManager::SPHERE3, pos);
            c3++;
        }
        else if (b.distToCamera <= 30 && b.distToCamera > 20)
        {
            // L2
            rm.Render(RenderManager::SPHERE2, pos);
            c2++;
        }
        else
        {
            // L1
            rm.Render(RenderManager::SPHERE1, pos);
            c1++;
        }
        UpdateBallPhysics(b);
    }
