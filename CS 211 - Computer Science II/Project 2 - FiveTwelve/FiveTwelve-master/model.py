...
class Vec():
    ...
    def __add__(self, other: "Vec") -> "Vec":
        new_vec = Vec((self.x + other.x), (self.y + other.y))
        return new_vec
...
class Board(GameElement):
    ...
    def __getitem__(self, pos: Vec) -> Tile:  # tile = board[pos]
    ...
    def from_list(self, values: List[List[int]]) -> None:
    ...
    def in_bounds(self, pos: Vec) -> bool:
        ...
        tf = False
        if (pos.x >= 0) and (pos.x < self.rows):
            if (pos.y >= 0) and (pos.y < self.cols):
                tf = True
        return tf
    ...
    def _move_tile(self, old_pos: Vec, new_pos: Vec):
        """Allows Board class to access Tile method move_to."""
    ...
    def right(self):
        dir = Vec(0, 1)
        for row in range(len(self.tiles)):
            for col in reversed(range(len(self.tiles))):
                self.slide(Vec(row, col), dir)
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                if self.tiles[row][col] is not None:
                    sum_elem += self.tiles[row][col].value
        return sum_elem
