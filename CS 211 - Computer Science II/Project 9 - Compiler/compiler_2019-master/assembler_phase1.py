    ...
    for lnum in range(len(lines)):
        ...
        try:
            ...
            if fields["kind"] == AsmSrcKind.FULL:
                ...
                log.debug("Passing through MEMOP instruction")
                ...
                transformed.append(line)
            if fields["kind"] is not AsmSrcKind.COMMENT:
                address += 1
...
def resolve(lines: List[str]) -> Dict[str, int]:
    ...
    for lnum in range(len(lines)):
        ...
        try:
            ...
            if fields["kind"] is not AsmSrcKind.COMMENT:
                # Increment address if not a label
                address += 1
        except Exception:
