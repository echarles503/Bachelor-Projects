...
class Expr(object):
    ...
    def gen(self, context: Context, target: str):
        raise NotImplementedError("gen method not defined for class")
...
class UnOp(Expr):
    ...
    def _opcode(self) -> str:
        """Which operation code do we use in the generated assembly code?"""
        raise NotImplementedError("Each unary operator should define the _opcode method")
...
class Read(Expr):
    ...
    def gen(self, context: Context, target: str):
        """We read a value by loading to the memory-mapped address 510"""
